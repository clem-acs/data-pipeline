...
class EyeGoodTransform(BaseTransform):
    ...
    def process_session(self, session: Session) -> Dict[str, Any]:
        ...
        # element selection
        # ------------------------------------------------------------------
        elems = extract_elements_data(eg, sid, logger=self.logger)
        if elems is None:
            return {"status": "skipped",
                    "metadata": {"reason": "no_elements", "session_id": sid}}

        # mask: task_type == "eye"  &  duration > S
        tt = elems.get("task_type")
        if tt is None:
            return {"status": "skipped",
                    "metadata": {"reason": "no_task_type", "session_id": sid}}

        task_type_str = tt.astype(str)
        durations_ms  = elems["end_time"] - elems["start_time"]
        qual_mask = (
            (task_type_str == "eye") &
            (durations_ms >= self.min_duration_seconds * 1000)
        )
        if not np.any(qual_mask):
            return {"status": "skipped",
                    "metadata": {"reason": "no_long_eye_elements", "session_id": sid}}

        # keep only qualifying rows
        qual_elems = {k: v[qual_mask] if isinstance(v, np.ndarray) else v
                      for k, v in elems.items()}

        # ------------------------------------------------------------------
        # window extraction: middle X-percent of each element
        # ------------------------------------------------------------------
        t_arr = wg["time"][:]
        hits: List[int] = []
        for s, e in zip(qual_elems["start_time"], qual_elems["end_time"]):
            dur = e - s
            mid_margin = (1.0 - self.middle_percent) / 2.0
            start_mid  = s + mid_margin * dur
            end_mid    = e - mid_margin * dur
            idx = window_indices_for_range(t_arr, start_mid, end_mid)
            if idx.size:
                hits.extend(idx)

        if not hits:
            return {"status": "skipped",
                    "metadata": {"reason": "no_window_hits", "session_id": sid}}

        hits = np.sort(np.unique(np.array(hits, dtype=np.int64)))

        # retrieve all arrays for hits
        windows = extract_full_windows(wg, hits, logger=self.logger)

        # ------------------------------------------------------------------
        # labels & element-ids (reuse helpers)
        # ------------------------------------------------------------------
        windows.update({
            "session_id": sid,
            "labels":      bytesify(build_labels(qual_elems, hits, t_arr,
                                                 self.label_map)),
            "element_ids": bytesify(build_eids  (qual_elems, hits, t_arr)),
        })

        # stats
        lbls  = windows["labels"] if "labels" in windows else np.empty(0)
        w_cnt = len(hits)
        e_cnt = len(qual_elems["element_ids"])
