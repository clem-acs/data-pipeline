...
Middle-X% neural windows from *listen* elements ≥ S seconds
→ processed/queries/beep_<S>_<X>.zarr
"""
...
LABELS = {"sound": 0, "quiet": 1, "intro": 2, "unknown": 3}
log = logging.getLogger(__name__)


class BeepTransform(BaseTransform):
    """Extract neural windows from middle X-percent of listen task elements."""
    ...
    def __init__(self, min_duration_seconds=9.0, middle_percent=0.5,
        ...
        super().__init__(transform_id=f"t4F_beep_v0_{int(self.S)}s_{self.X}", 
                         script_id="4F", script_name="beep", script_version="v0", **kw)

    # ---------- CLI ----------
    @classmethod
    def add_subclass_arguments(cls, p):
        p.add_argument("--min-duration-seconds", type=float, default=9)
        p.add_argument("--middle-percent",      type=float, default=0.5)
    ...
    def process_session(self, s: Session) -> Dict[str, Any]:
        ...
        win = extract_elements_neural_windows(eg, wg, sid,
                filter_kwargs={"task_type":"listen"}, label_map=self.label_map,
        ...
        out=f"{self.DEST_PREFIX}beep_{int(self.S)}_{self.X}.zarr"
        if not self.dry_run:
            root,sgrp,_=init_or_open_result_store(self.s3,b,out,"beep",self.label_map,log)
...
if __name__ == "__main__":
    BeepTransform.run_from_command_line()